...
namespace ZooManager
{
    public static class Game
    {
        ...
        static public void AddToHolding(string occupantType)
        {
            ...
            if (occupantType == "alien") holdingPen.occupant = new Alien();
        ...
        static public void ActivateAnimals()
        {
            for (var y = 0; y < numCellsY; y++)
            {
                for (var x = 0; x < numCellsX; x++)
                {
                    ...
                    if (zone.occupant as Alien != null && !((Alien)zone.occupant).move)
                    {
                        ((Alien)zone.occupant).move = true;
            ...
            for (var r = 1; r < 11; r++) // reaction times from 1 to 10
            {
                for (var y = 0; y < numCellsY; y++)
                {
                    ...
                    {
                        var zone = animalZones[y][x];
                        if (zone.occupant as Animal != null && ((Animal)zone.occupant).reactionTime == r && !((Animal)zone.occupant).move)
                        {
                            ((Animal)zone.occupant).move = true;
